...
The plan lives as markdown in `content/` and tabular data in `data/`. A single self-contained web page is generated into `site/index.html`, and polished Word/Excel versions into `deliverables/`.

## Quick start
```bash
pip install markdown openpyxl
python3 build/build_site.py     # regenerate site/index.html
...
- `site/` — the public web page (`index.html`) + `PUBLISH.md` (GitHub Pages steps).
- `deliverables/` — generated Word/PDF/Excel versions.

See **`CLAUDE.md`** for the full map: the source-of-truth rule, all build commands, and the visual/brand conventions. See **`site/PUBLISH.md`** to put the page live on GitHub Pages.
